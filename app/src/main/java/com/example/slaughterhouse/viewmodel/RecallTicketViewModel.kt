...
@HiltViewModel
class RecallTicketViewModel @Inject constructor(private val apiRepository: Repository) :
    ViewModel() {

    val result = MutableLiveData<Resource<Response<Unit>?>>()


    fun recallTicket(counter: String,userid: String,ticketNo: String,ticketId: String,refNo: String){
        viewModelScope.launch {
            result.postValue(Resource.Loading(true))
            val response = apiRepository.recallTicket(counter, userid, ticketNo, ticketId, refNo)
